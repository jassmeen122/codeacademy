...
export const updateSkillsFromActivity = async (
  ...
  try {
    for (const skillName of skillsToUpdate) {
      ...
      if (upsertError) {
        console.error(`Error updating skill ${skillName}:`, upsertError);
      }
    }
  } catch (error) {
...
export const awardBadgeForProgress = async (userId: string, skillName: string, progress: number) => {
  ...
  const badges: Record<string, number> = {
    [`Débutant ${skillName}`]: 25,
    [`Intermédiaire ${skillName}`]: 50,
    [`Avancé ${skillName}`]: 75,
    [`Expert ${skillName}`]: 100
  };
  
  for (const [badgeName, requiredProgress] of Object.entries(badges)) {
