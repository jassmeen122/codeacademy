...
export const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      // If user exists, ensure they're on the right dashboard
      if (user) {
        const currentPath = window.location.pathname;
        const expectedBasePath = `/${user.role}`;
        
        // If user is on wrong dashboard, redirect them
        if (!currentPath.startsWith(expectedBasePath) && 
            !currentPath.startsWith('/auth') && 
            currentPath !== '/') {
          console.log(`Redirecting ${user.role} from ${currentPath} to ${expectedBasePath}`);
          navigate(expectedBasePath);
        }
      }
    };
